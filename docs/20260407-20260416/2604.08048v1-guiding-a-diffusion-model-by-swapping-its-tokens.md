...
tags: ["query:cv-vg-cs"]
score: 7.0
evidence: 无需重新训练的扩散引导推理技术
tldr: 针对分类器自由引导（CFG）依赖文本条件的局限性，本文提出了一种名为Self-Swap Guidance (SSG) 的通用引导方法。该方法通过在空间或通道维度交换语义差异最大的Token潜变量来产生扰动预测，并利用其与原始预测的差值引导采样过程。SSG不仅适用于有条件生成，也支持无条件生成，在提升图像质量和提示词对齐度方面优于现有方法，且作为插件可轻松集成到各类扩散模型中。
source: arxiv
...
method: 通过在空间或通道维度交换语义最不相似的Token潜变量对来构造扰动预测，并以此引导采样向高保真分布靠拢。
result: 在MS-COCO和ImageNet等数据集上的实验表明，SSG在图像保真度、提示词对齐度及鲁棒性方面均优于现有的无条件引导方法。
conclusion: SSG是一种简单高效、无需额外训练的即插即用方法，成功将类CFG引导扩展到了更广泛的生成场景。
---

## 摘要
无分类器引导（Classifier-Free Guidance, CFG）是一种广泛使用的推理阶段技术，用于提升扩散模型的图像质量。然而，它对文本条件的依赖限制了其在无条件生成中的应用。我们提出了一种简单的方法，使有条件和无条件生成都能实现类似 CFG 的引导。其核心思想是通过简单的 Token 交换操作生成扰动预测，并利用该预测与原始预测之间的方向，引导采样过程向更高保真度的分布靠拢。在实践中，我们在空间或通道维度上交换语义差异最大的 Token 潜变量对。与现有以全局或较少约束方式应用扰动的方法不同，我们的方法选择性地交换和重组 Token 潜变量，从而实现对扰动及其对生成样本影响的更精细控制。在 MS-COCO 2014、MS-COCO 2017 和 ImageNet 数据集上的实验表明，所提出的自交换引导（Self-Swap Guidance, SSG）在应用于主流扩散模型时，在不同设置下的图像保真度和提示词对齐方面均优于以往的无条件引导方法。其细粒度的扰动粒度还提高了鲁棒性，在更广泛的扰动强度范围内减少了副作用。总之，SSG 将 CFG 扩展到了包括有条件和无条件生成在内的更广泛应用范围，并且可以作为插件轻松插入任何扩散模型中，以获得即时的性能提升。
