...
tldr: 本研究针对预训练扩散模型在处理超训练分辨率图像时出现的结构失真和内容重复问题，提出了无需微调的图像编辑框架EditCrafter。该方法通过分块反演技术保留输入图像的原始身份，并引入噪声衰减流形约束无分类器指导（NDCFG++）来优化高分辨率下的潜空间编辑。EditCrafter支持任意长宽比的高清图像编辑，在保持生成质量的同时显著提升了模型的适用范围。
source: arxiv
...
motivation: 现有的扩散模型编辑方法受限于训练分辨率，在处理高分辨率或任意比例图像时容易产生结构崩坏和内容重复。
method: 提出了一种结合分块反演（Tiled Inversion）与噪声衰减流形约束无分类器指导（NDCFG++）的免微调编辑流水线。
result: 实验表明，EditCrafter无需任何微调或优化即可在多种高分辨率和长宽比下实现出色的图像编辑效果。
conclusion: EditCrafter为高分辨率图像编辑提供了一种简单且有效的通用解决方案，充分发挥了预训练扩散模型的生成潜力。
---
